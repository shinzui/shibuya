...
Prefetch deadlock reproduction notes (for future upstream investigation; kept here as context, not as a living discovery). In the adapter repository, set `prefetchConfig = Just defaultPrefetchConfig` on any consumer config in `shibuya-pgmq-example/app/Consumer.hs` (e.g., `notificationsAdapterConfig`), start PostgreSQL per that repo's `process-compose.yaml`, run `cabal run shibuya-pgmq-simulator` and `cabal run shibuya-pgmq-consumer`: the consumer stops making progress once `parBuffered`'s channel is involved, typically dying with `thread blocked indefinitely in an STM transaction`. The suspected mechanism: `StreamP.parBuffered` spawns worker threads through streamly's `MonadAsync` machinery, which requires unlifting `Eff es` to `IO`; effectful's unlifting strategy for the dynamically dispatched `Pgmq`/`Error` effects serializes or fails to start the worker, so the consumer side blocks forever on the channel's STM read while no producer ever runs. The bug is in the interaction, not in pgmq: any `Eff`-based stream under `parBuffered` with this effect stack should reproduce it. Fixing it means investigating streamly's channel + effectful's `withEffToIO` upstream — out of scope here (see Decision Log).
