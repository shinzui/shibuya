...
2026-09-20: Merely moving the terminal branch behind `receiveSTM` was insufficient: constructing the extra `orElse` alternatives added roughly 100--130 allocated bytes per message on the serial path. Splitting the empty check and wakeable wait into two transactions removed that allocation but made a frequently empty retry workload pay two transactions per message. Intake now uses one conditional transaction: a populated inbox checks and receives without constructing wake alternatives or reading the terminal `TVar`; only an observed-empty transaction reads the terminal and source-completion variables before retrying. The inbox, terminal request, or source completion then reruns that same transaction.
