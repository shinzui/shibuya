...
- 2026-07-03 (EP-27, post-release): the PGMQ prefetch deadlock removed in 0.9.0.0 was root-caused and is **not** a streamly bug, contrary to this MasterPlan's Scope-boundary framing. `parBuffered` forks producer worker threads that must unlift `Eff es`; effectful's default `SeqUnlift` strategy `error`s when its unlift runs off-thread (`effectful-core/.../Internal/Monad.hs:214`), so the producer dies and the consumer hangs on the channel STM read. Both effectful instances satisfying `MonadAsync (Eff es)` read the strategy dynamically (`MonadUnliftIO`/`MonadBaseControl` at lines 433/449), so scoped `withUnliftStrategy (ConcUnlift Ephemeral Unlimited)` applied via `morphInner` to only the prefetch sub-stream fixes it without touching the non-prefetch path or streamly. Note this is genuinely adapter-local: core's own `ConcUnlift Persistent` around `adapter.source` (above) does **not** cover it, because `ConcUnlift` clones the original env (`cloneEnv es0`, `Effectful/Internal/Unlift.hs:159/171`) which preserves the ambient `SeqUnlift`, so the adapter's nested `parBuffered` still reads `SeqUnlift`. The removal still stands for 0.9.0.0; re-enabling is deferred to an evidence-gated prototype living in the adapter repository's own plan series (`shibuya-pgmq-adapter/docs/plans/3-prototype-re-enabling-pgmq-prefetch-via-scoped-concunlift.md`), which is intentionally *not* a child of this MasterPlan (feature-revival work, out of this remediation initiative's scope).
...
- Decision: The PGMQ prefetch re-enablement follow-up (scoped `ConcUnlift` fix) is authored as a standalone plan in the adapter repository's own series (`shibuya-pgmq-adapter/docs/plans/3-prototype-re-enabling-pgmq-prefetch-via-scoped-concunlift.md`), not as a child of this MasterPlan, and is deliberately excepted from the "all child plans live in core" convention above.
  Rationale: This initiative is remediation-scoped and explicitly excluded fixing the prefetch deadlock (see Scope boundary); reviving prefetch is new feature work discovered after the fact. The adapter already maintains an independent numbered plan series, the work is entirely adapter-local (no core change), and it is evidence-gated (must pass a deadlock-reproduction test before shipping). Cross-references in EP-27 and this Surprises log keep it discoverable without pulling it into the initiative's registry.
  Date: 2026-07-03
...
Revision note, 2026-07-03 (EP-27 post-release correction): recorded that the PGMQ prefetch deadlock (removed in adapter 0.9.0.0) is an effectful `SeqUnlift` off-thread-unlift error, not a streamly bug, and is fixable adapter-locally with scoped `ConcUnlift` — superseding the Scope-boundary framing that treated it as an upstream-streamly problem. Added a Surprises entry (with the `cloneEnv`/`SeqUnlift` evidence showing core's own `ConcUnlift Persistent` does not cover the adapter's nested `parBuffered`) and a Decision Log entry deferring the fix to a standalone, evidence-gated prototype in the adapter repository's plan series (`shibuya-pgmq-adapter/docs/plans/3-prototype-re-enabling-pgmq-prefetch-via-scoped-concunlift.md`), intentionally excepted from the initiative registry. The shipped 0.9.0.0 outcome and all child-plan statuses are unchanged.
