...
- Start the NQE supervisor without linking it to the thread that called `runApp`.
...
- Deliver a processor failure to the caller exactly once under `StopAllOnFailure`.
...
- Add a second process-isolated garbage-collection regression,
